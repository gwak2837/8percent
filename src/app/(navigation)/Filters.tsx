...
export default function Filters({ titles }: Props) {
  ...
  function handleRemoveFilter(key: (typeof FILTER_KEYS)[number]) {
    const newSearchParams = new URLSearchParams(Array.from(searchParams.entries()))
  ...
  return (
    <div className="grid gap-2">
      <label className="peer flex cursor-pointer items-center gap-2 md:hidden">
      ...
      <div className="flex flex-col gap-6 whitespace-nowrap rounded-xl bg-gray-100 p-4 peer-has-[:checked]:hidden dark:bg-gray-800">
        ...
        {[
          ...
          <div className="grid grid-cols-[1fr_auto] gap-3" key={filter.key}>
            ...
            <div className="col-span-full grid max-h-[20vh] grid-cols-[repeat(auto-fill,minmax(90px,1fr))] gap-3 gap-y-2 overflow-y-auto rounded-xl bg-white p-3 px-4 peer-has-[:checked]:hidden md:max-h-[50vh] dark:bg-gray-700">
